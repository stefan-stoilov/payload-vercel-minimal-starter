...
This template comes pre-configured with the official [Payload MCP Plugin](https://payloadcms.com/docs/plugins/mcp), which exposes your content over the [Model Context Protocol](https://modelcontextprotocol.io) so AI clients (such as Claude Code, Cursor, or VS Code) can read and manage it. The plugin registers a JSON-RPC 2.0 endpoint at `POST /api/mcp`.

The following are enabled by default (see `src/plugins/index.ts`): `pages`, `posts`, `categories` and `media` with full CRUD, plus the `header` and `footer` globals with find/update. The `users` collection is intentionally not exposed. As an extra safeguard, the homepage (the page with slug `home`) cannot be deleted over MCP — see `src/collections/Pages/hooks/protectHomepageFromMcp.ts`.

### Create an API key

MCP requests are authenticated with a Bearer token:

1. Start the app and open the admin panel at `/admin`.
2. Go to **MCP → API Keys** and create a key.
3. Toggle the capabilities you want this key to allow per collection/global.
4. Copy the generated API key.

Requests must include the header `Authorization: Bearer <MCP_API_KEY>`. You can store the value under `MCP_API_KEY` in your `.env` for convenience (it is not read by the app itself).
...
You can verify the endpoint directly with curl:
...
`@payloadcms/plugin-mcp@3.82.1` ships a bug where the Create/Update tools fail to register (`convertedFields.partial is not a function`): its schema converter evaluates `new Function('z', 'return ' + transpiledCode)`, but `ts.transpileModule` prepends a `"use strict";` directive, so the function returns the string `"use strict"` instead of a Zod schema. We carry a one-line fix in `patches/@payloadcms__plugin-mcp@3.82.1.patch` (applied automatically by pnpm via `patchedDependencies` in `package.json`). The bug is still present in the latest release (3.85.1) as of this writing — **remove the patch once it is fixed upstream.**
